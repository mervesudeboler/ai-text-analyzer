...
class TextAnalyzer:
    ...
    Capabilities
    ------------
    - Tokenisation & stop-word removal (configurable language)
    ...
    def __init__(self, lang: str = "en") -> None:
        ...
        if lang_name != "english":
            self.stop_words |= set(stopwords.words(lang_name))
        self.sia = SentimentIntensityAnalyzer()
        logger.info("TextAnalyzer initialised (lang=%s).", lang)

    # Private helpers

    def _bootstrap_nltk(self) -> None:
        """Download any missing NLTK resources (checked once, not every run)."""
        for path, package in self.NLTK_RESOURCES.items():
            try:
                nltk.data.find(path)
            except LookupError:
    ...
    def analyze(self, text: str, top_n: int = 15) -> dict:
        ...
        Raises
        ------
        ValueError : if text is empty or contains no analysable words
        """
        ...
        if not words:
            raise ValueError(
                "No analysable words found. "
                "Make sure the text contains alphabetic content."
            )

        if not filtered:
            raise ValueError(
                "All words were filtered out as stop-words. "
                "Try a longer or more content-rich text."
            )
        ...
        return {
            ...
            "metrics": {
                ...
                "lexical_density": round(len(set(words)) / len(words), 4),
                "sentiment": {
                    **sentiment,
    ...
    def generate_visuals(self, results: dict, output_dir: str = "outputs") -> None:
        ...
        try:
            ...
            logger.error("Failed to generate bar chart: %s", exc)
        ...
        try:
            ...
            logger.error("Failed to generate word cloud: %s", exc)
    ...
    def export_json(self, results: dict, output_dir: str = "outputs") -> str:
        ...
        export_data = {
            **results,
            "top_words": [{"word": w, "count": c} for w, c in results["top_words"]],
        }
        try:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  python main.py --file makale.txt --lang tr --export-json\n"
    ...
    parser.add_argument("--lang",        type=str, default="en",
                        ...
                        help="Language for stop-word filtering (default: en)")
...
def main() -> None:
    ...
    try:
        analyzer = TextAnalyzer(lang=args.lang)
    ...
    try:
        if args.file:
            ...
            print("Paste your text below (press Ctrl+D when done):")
            content = sys.stdin.read()
        ...
        if args.visualize:
            ...
            print(f"  Visuals saved to: {args.output_dir}/")

        if args.export_json:
            path = analyzer.export_json(results, output_dir=args.output_dir)
